...
void keyboard_interrupt() {
	...
	//group_process_id = getpgid(getpid());
	//kill(-1, SIGTERM); //Sends the signal sig to all processes whose process group ID is equal to the absolute value of pid
	//sleep(1);
	//printf("parent[%d]: Terminated %d processes.\n", (int) getpid(), CREATED_CHILDS); //Termination
			
  	//for(int j=1; j<NSIG; j++) //The old service handers of all signals should be restored
		//signal(j, SIG_DFL);
	...
	for (int i = 0; i < n; ++i) {
		...
		if (child_pid > 0) {
			...
			if (interrupt_flag == 1){
				printf("parent[%i]: Interrupt of the creation process!\n", getpid());
				group_process_id = getpgid(getpid());
				kill(-1 * group_process_id, SIGTERM); //Sends the signal sig to all processes whose process group ID is equal to the absolute value of pid
				sleep(1);
				printf("parent[%d]: Terminated %d processes.\n", (int) getpid(), CREATED_CHILDS); //Termination
...
int main(int argc, char ** argv) {
	...
	#ifdef WITH_SIGNALS
		for(int j = 1; j < NSIG; ++j) // NSIG the total number of signals defined
